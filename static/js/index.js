...
function world_static_list() {
	...
	$.ajax({
		...
		success: function (data) {  //如果请求成功，则执行success对应的function,data会接收上一行请求url的返回值
			$("#china_total").text(data['total'][0]);  //中国累计接种
			$("#china_new").text(data['new'][0]);  // 中国新增接种
			$("#china_per").text(data['per'][0]);  // 中国每百人接种
			$("#word_per").text(data['per'][1]);  // 全球每百人接种
			$("#word_new").text(data['new'][1]);  // 全球新增接种
			$("#word_total").text(data['total'][1]);  //全球累计接种
...
// 执行
$(function() {
	setInterval(flashAll, 1000 * 60 * 10)  //每隔30分钟调用flashAll函数，刷新页面数据
